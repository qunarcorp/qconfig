...
public class MapValueChecker extends AbstractCollectionValueChecker<Map.Entry<String, String>> {

    private final static Splitter SPLITTER = Splitter.on(';').trimResults().omitEmptyStrings();

    public MapValueChecker(ObjectNode node) {
        super(node);
    }
    ...
    protected Collection<Map.Entry<String, String>> parseElements(String value) {
        ...
        for (String kvPair : entries) {
            ...
            Preconditions.checkArgument(index >= 0, String.format("%s的值中key:value缺少\":\"分隔符", name()));
            String key = kvPair.substring(0, index);
            String val = kvPair.substring(index + 1);
            Preconditions.checkArgument(!Strings.isBlank(key), String.format("%s的值中key不能为空", name()));
            if (map.put(key, val) != null) {
                throw new IllegalArgumentException(String.format("%s的值中key[%s]重复", name(), key));
            }
        }
        return map.entrySet();
    }
